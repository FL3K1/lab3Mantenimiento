...
class InvoiceLineCreateSerializer(serializers.ModelSerializer):
    ...
    # --- CÓDIGO AÑADIDO PARA EL EJERCICIO 2 ---
    def validate(self, attrs):
        barrel = attrs.get('barrel')
        invoice = self.context.get('invoice')

        if barrel and invoice:
            if barrel.provider_id != invoice.provider_id:
                raise serializers.ValidationError({
                    'barrel': 'Barrel must belong to the same provider as the invoice.'
                })
        return attrs
    # ------------------------------------------
